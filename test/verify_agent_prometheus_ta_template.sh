...
# 5. Collector config has the merged-pipeline transform processor (replaces
#    filter/pod_metrics + filter/cadvisor of the earlier two-pipeline design).
echo "${COLLECTOR_CM}" | grep -q 'transform/set_debug_source' \
  && pass "Collector has transform/set_debug_source processor" \
  || fail "Collector missing transform/set_debug_source processor"
echo "${COLLECTOR_CM}" | grep -q 'filter/pod_metrics' \
  && fail "Collector still has filter/pod_metrics (should be merged out)" \
  || pass "Collector has no filter/pod_metrics (merged-pipeline design)"
echo "${COLLECTOR_CM}" | grep -q 'metrics/k8s_metrics:' \
  && pass "Collector has merged metrics/k8s_metrics pipeline" \
  || fail "Collector missing merged metrics/k8s_metrics pipeline"
