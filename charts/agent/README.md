...
| application.prometheusScrape.targetAllocator.prometheusCR.podMonitorNamespaceSelector | object | `{}` | PodMonitor namespace selector. Empty (default) matches all namespaces. Same "only-emit-when-set" behavior as serviceMonitorNamespaceSelector. |
| application.prometheusScrape.targetAllocator.prometheusCR.podMonitorSelector | object | `{}` | PodMonitor label selector. Empty (default) matches all PodMonitors. Same "required when enabled" caveat as serviceMonitorSelector. |
| application.prometheusScrape.targetAllocator.prometheusCR.serviceMonitorNamespaceSelector | object | `{}` | ServiceMonitor namespace selector. Empty (default) matches all namespaces. The chart only emits this key when set to a non-empty value, so default render is byte-identical to omitting the field. |
| application.prometheusScrape.targetAllocator.prometheusCR.serviceMonitorSelector | object | `{}` | ServiceMonitor label selector. Empty (default) matches all ServiceMonitors. Note: TA treats this field as required when prometheusCR.enabled=true — omitting it would cause TA to match nothing — so the chart always emits it, defaulting to `{}`. |
